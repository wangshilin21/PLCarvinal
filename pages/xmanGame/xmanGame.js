...
Page({
  ...
  enterGame: function() { //报名阶段
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        wx.setStorageSync('playerMe', app.globalData.playerMe);
        ...
        if (that.data.resResult == 0) {
          //console.log("报名成功！")
          app.globalData.interval = 404;
          app.globalData.interval1 = 404;
          app.globalData.interval2 = 404;
          wx.showToast({
            title: 'success',
            icon: 'success',
            duration: 2000,
          ...
          setTimeout(function() {
            ...
            wx.setStorageSync('joinState', that.data.joinState);
            that.onLoad();
          }, 2000)
  ...
  onShow: function(options) {
    ...
    var logLock4 = false;
    var joinStateSync = 0;
    var wordHasGottenSync = false;
    ///joinState 緩存處理
    if (wx.getStorageSync('joinState')) {
      joinStateSync = wx.getStorageSync('joinState');
    }
    that.setData({
      joinState: joinStateSync
    });
    ///wordHasGotten 緩存處理
    if (wx.getStorageSync('wordHasGotten')) {
      wordHasGottenSync = wx.getStorageSync('wordHasGotten');
    }
    that.setData({
      wordHasGotten: wordHasGottenSync
    });
    ...
    wx.setNavigationBarTitle({
      title: 'Planning Manager Carnival'
    })

    app.globalData.interval = setInterval(function() {
      if (app.globalData.interval == 0) {
        return
      };
      ...
      if (app.globalData.gameState != 0) {
        if (that.data.joinState == 1) {
          console.log("+++++Show Toast is "+that.data.showToast);
          if(that.data.showToast== true){
            that.data.showToast=false;
            console.log("Show Toast is "+that.data.showToast);
          wx.showToast({
            title: 'Connecting...',
            icon: 'loading',
            ...
            success: function(res) {
              ...
              if (app.globalData.gameState != 0) {
                ...
                if (app.globalData.cardMe.length > 0) {
                  ...
                  wx.setStorageSync('wordHasGotten', that.data.wordHasGotten);
   ...
   */
  onHide: function() {
    console.log("-=-=-=-=-=-=-=-=已成功啟動On Hide 函數-=-=-=-=-===-XmanGame界面");
   ...
   */
  onUnload: function() {
    console.log("-=-=-=-=-=-=-=-=已成功啟動On Unload 函數-=-=-=-=-===-XmanGame界面");
    clearInterval(app.globalData.interval);
    app.globalData.interval = 0;
    console.log("clear Interval");
  },
